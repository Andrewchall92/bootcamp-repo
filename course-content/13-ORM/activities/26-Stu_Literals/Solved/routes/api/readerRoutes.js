...
router.get('/test', async (req, res) => {
  try {
    const readerData = await Reader.findAll({
      include: [{ model: LibraryCard }, { model: Book, attributes: {
        include: [
          [
            // Use plain SQL to get a count of all short books
            sequelize.literal(
      ...
      attributes: {
        include: [
          ...
          [
            // Use plain SQL to get a count of all short books
            sequelize.literal(
              '(SELECT COUNT(*) FROM book WHERE pages BETWEEN 300 AND 1000 AND book.reader_id = reader.id)'
            ),
            'longBooks',
          ],
          [
            ...
            // Use plain SQL to get a count of all short books
            sequelize.literal(
              '(SELECT COUNT(*) FROM book WHERE pages > 1000 AND book.reader_id = reader.id)'
            ),
            'superLongBooks',
          ]
        ],
